...
export default function BenchmarkPage() {
  ...
  const validation = useMemo(() => {
    ...
    if (data.fps_avg > 1000) issues.push('Average FPS looks too high.');
    if (data.fps_1_low !== undefined) {
      if (data.fps_1_low <= 0) issues.push('1% low must be greater than 0.');
      if (data.fps_1_low > data.fps_avg) issues.push('1% low should be below average.');
    }
