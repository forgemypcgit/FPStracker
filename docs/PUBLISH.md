...
Set `COSIGN_PRIVATE_KEY` to the full contents of `cosign.key`.
...
- Workflow updates `manifests/p/PCBuilder/FPSTracker/<version>/`
...
1. Download `fps-tracker.rb` and `PCBuilder.FPSTracker*.yaml` from release assets.
