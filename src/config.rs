...
impl Config {
    ...
    pub fn load() -> Result<Self> {
        ...
        let content = fs::read_to_string(&primary)
            .with_context(|| format!("Failed to read config from {}", primary.display()))?;

        let config: Config = toml::from_str(&content)
            .with_context(|| format!("Failed to parse config from {}", primary.display()))?;
    ...
    pub fn config_path() -> Result<PathBuf> {
        ...
        let proj_dirs = ProjectDirs::from("com", "forgemypc", "fps-tracker")
...
fn default_api_url() -> String {
    "https://fps-tracker-api-689034767510.us-central1.run.app".to_string()
}
...
mod tests {
    ...
    fn test_default_config() {
        ...
        assert_eq!(
            config.api.base_url,
            "https://fps-tracker-api-689034767510.us-central1.run.app"
        );
