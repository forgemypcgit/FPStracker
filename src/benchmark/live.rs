...
fn resolve_source(options: &LiveCaptureOptions) -> Result<CaptureSource> {
    ...
    #[cfg(target_os = "windows")]
    {
        if is_tool_available("presentmon") {
            return Ok(CaptureSource::PresentMon);
        }
    }

    if options.file.is_some() || import::mangohud::find_latest_mangohud_log().is_some() {
...
fn capture_from_mangohud(options: &LiveCaptureOptions) -> Result<LiveCaptureResult> {
    ...
    {
        anyhow::bail!(
            "Strict focus policy requires --process-name for MangoHud live capture.\
            Re-run with --process-name <game.exe> or use --focus-policy lenient."
        );
    }
    ...
    let mut offset = File::open(&capture_path)
...
fn capture_from_presentmon(options: &LiveCaptureOptions) -> Result<LiveCaptureResult> {
    ...
    {
        if !is_tool_available("presentmon") {
            anyhow::bail!("presentmon is not available. Install PresentMon and add it to PATH.");
        }
        ...
        let mut diagnostics = CaptureDiagnostics {
            target_process: options.process_name.clone(),
            process_validation_enabled: options.process_validation
                && options.process_name.is_some(),
            ..CaptureDiagnostics::default()
        };
        ...
        let mut child = cmd.spawn().context("Failed to run presentmon")?;
        ...
        if !status.success() {
            anyhow::bail!("PresentMon failed with status {}", status);
        }

        if options.pause_on_unfocus
            && options.focus_policy == FocusPolicy::Strict
            && focus_tracker.strict_focus_violation()
        {
            anyhow::bail!(
                "Strict focus policy blocked this capture due to sustained focus loss (> {}ms).\
...
fn build_result(
    ...
    let valid_frame_times: Vec<f64> = frame_times_ms
        .into_iter()
        .filter(|value| {
            value.is_finite() && *value > 0.0 && *value <= max_frame_time_ms && *value <= 1000.0
        })
        .collect();
...
fn parse_frametime(value: &str) -> Option<f64> {
    ...
    if parsed.is_finite() && parsed > 0.0 && parsed <= 1000.0 {
        Some(parsed)
    } else {
        None
    }
...
impl FocusTracker {
    ...
    fn should_collect(&mut self) -> bool {
        ...
        if self.last_probe.elapsed() < Duration::from_millis(400) {
            return self.cached_collect;
        }

        self.last_probe = Instant::now();

        let collect_now = match self.target_process_normalized.as_deref() {
            Some(target) => match focus::foreground_process_name() {
                Some(active) => {
                    let is_match = focus::process_name_matches(&active, target);
                    if !is_match {
                        self.mismatch_detected = true;
                    }
                    is_match
                }
                // Unknown foreground process should not fail strict mode by itself.
                None => true,
            },
            None => self.focus_policy == FocusPolicy::Lenient,
        };
...
mod tests {
    use super::{
        ...
        parse_frametime, CaptureDiagnostics, FocusPolicy, FocusTracker, MangoHudStreamParser,
    };
